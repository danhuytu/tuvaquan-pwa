(() => {
  ...
  async function api(url, options = {}) {
    ...
    if (!response.ok) {
      const fallback = response.status === 401 ? 'Phiên đăng nhập đã hết. Hãy đăng nhập lại.'
        : response.status === 429 ? 'Bạn thao tác quá nhanh. Hãy đợi vài giây rồi thử lại.'
        : response.status === 403 ? 'Bạn không có quyền thực hiện thao tác này.'
        : response.status >= 500 ? 'Máy chủ đang gặp lỗi tạm thời. Thư chưa được gửi, hãy thử lại sau ít giây.'
        : 'Có lỗi xảy ra. Vui lòng thử lại.';
      throw new Error(body.message || body.title || fallback);
    }
    return body;
  }
  ...
  $('#composeForm').addEventListener('submit', async event => { event.preventDefault(); const submit = event.target.querySelector('button[type="submit"]'); if (submit.disabled) return; submit.disabled = true; const label = submit.textContent; submit.textContent = 'Đang gửi...'; try { await api('/api/letters', { method: 'POST', body: JSON.stringify({ recipient: normalized($('#recipient').value), type: $('#letterType').value, content: $('#letterContent').value.trim() }) }); $('#composeMessage').style.color = '#438258'; $('#composeMessage').textContent = 'Lá thư đã được gửi đi.'; setTimeout(closeModal, 700); } catch (error) { $('#composeMessage').style.color = ''; $('#composeMessage').textContent = error.message; } finally { submit.disabled = false; submit.textContent = label; } });
